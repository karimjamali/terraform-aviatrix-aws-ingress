...
module "security_group_proxy_vm" {
  ...
  ingress_cidr_blocks = [var.home_ip]
...
# Reading the interface details about the Proxy LB
/*
data "aws_network_interface" "proxy-lb-1" {  
    filter {
      name   = "description"
...
data "aws_network_interface" "proxy-lb-2" {  
    filter {
      name   = "description"
      values = ["ELB ${aws_lb.proxy_lb.arn_suffix}"]
    }
...
resource "aws_lb" "proxy_lb" {
  ...
  security_groups    = [module.security_group_proxy_vm.this_security_group_id]
  subnets            = [for subnet in module.spoke_aws-us-east-1-proxy.vpc.public_subnets : subnet.subnet_id]
